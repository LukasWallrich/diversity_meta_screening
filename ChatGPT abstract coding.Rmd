---
title: "Use ChatGPT to extract data from academic abstracts"
output: html_notebook
---

ChatGPT is used to extract information from the abstracts - obviously, not to replace human coding but solely to accelerate the process by providing suggestions for some fields that can be tedious to type (e.g., country).

# Code abstracts

Cost ~2 USD for 1800 abstracts.
...
resp_df %>% qs::qsave("chatgpt_out_df.qs")
...
# Review proposed exclusions

Review entries that ChatGPT flagged as not quantitative empirical and manually exclude where categorisation is correct - usually right, turned out to be review chapters including quantitative statements.

```{r}

included_aug %>% mutate(ID = coalesce(doi, duplicate_id)) %>% left_join(resp_df) %>% filter(quant == "no", is.na(retrieved) | retrieved == "pdf_downloaded_victoria") %>% select(ID, title, abstract) %>% pull(ID)
